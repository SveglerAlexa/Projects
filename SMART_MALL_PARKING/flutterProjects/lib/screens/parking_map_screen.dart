...
import '../screens/parter_map_screen.dart';

class ParkingMapScreen extends StatefulWidget {
  final String floor;

  const ParkingMapScreen({super.key, this.floor = 'Exterioară'});
...
class _ParkingMapScreenState extends State<ParkingMapScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Parcare ${widget.floor}'),
        backgroundColor: Colors.white,
        foregroundColor: Colors.black,
        elevation: 0,
        actions: [
          if (widget.floor == 'Exterioară')
            TextButton.icon(
              onPressed: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) =>
                    const ParkingMapInsideScreen(floor: 'Parter'),
                  ),
                );
              },
      ...
      body: FutureBuilder<List<ParkingSpotFromApi>>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Center(
              child: Text(
                'Eroare la încărcarea locurilor: ${snapshot.error}',
              ),
            );
          ...
          // 🔹 Filtrare DOAR exterior
          final spots = snapshot.data!
              .where((spot) {
            final etajInt = int.tryParse(spot.etaj) ?? -1;
            return etajInt == 0;
          }).toList();
